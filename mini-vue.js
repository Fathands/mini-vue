...
class Compile {
	...
	compileText(node, expression) {
		this.updataDomVal(node, expression, "textContent", this.$vm[expression]);
	}
	// 编译innerHTML节点
	compileInnerHTML(node) {
		Object.keys(node.attributes).forEach(key => {
			let expression = node.attributes[key]["nodeValue"];
			let val = this.$vm[node.attributes[key]["nodeValue"]];
			// 普通指令渲染
			switch (node.attributes[key]["nodeName"]) {
				case "v-text":
					this.updataDomVal(node, expression, "textContent", val);
					break;
				case "v-html":
					this.updataDomVal(node, expression, "innerHTML", val);
					break;
			}
			// 事件指令解析
	...
	// 更新节点
	updataDomVal(node, expression, domType, domValue) {
		// 标记每个使用data属性对象的dom节点位置, 并一直监听，当有变化时，会被dep实例捕获
		new Watcher(this.$vm, node, expression, (newVal, oldVal) => {
			node[domType] = newVal;
    });
...
// 创建监听类，监听每个渲染数据地方
class Watcher {
	constructor(vm, node, expression, callback) {
		// 每个watcher的唯一标识
		this.id = uid++;
		this.$vm = vm;
		// 每个watcher监听节点
	...
	addDep(dep) {
		if (!this.newDepIds.has(this.id)) {
      this.newDepIds.add(this.id)
			dep.addSub(this);
		}
	}
	updata(newVal) {
